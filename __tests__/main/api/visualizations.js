...
describe("POST /visualizations - new visualization", () => {
    test("creates a new visualization in database, sends 201 status code and id", async () => {
        ...
        const res = await request(api).post('/visualizations').set("Authorization", `Bearer ${loginDetails.token}`).send(VISUAL_POST_REQ_BODY)
    ...
    test("sends 400 status code and error when name is not given", async () => {
        ...
        axios.post.mockImplementation(() => Promise.resolve(MOCK_VISUAL_API_POST_RES_BODY))
        const res = await request(api).post('/visualizations').set("Authorization", `Bearer ${loginDetails.token}`).send({})
...
describe("GET /visualizations/{id} - get visualization info", () => {
    test("sends appropriate response body and 200 status code", async () => {
        const loginDetails = await registerAndLogin(TEST_USER)  
        axios.post.mockImplementation(() => Promise.resolve(MOCK_VISUAL_API_POST_RES_BODY))
        const createRes = await request(api).post('/visualizations').set("Authorization", `Bearer ${loginDetails.token}`).send(VISUAL_POST_REQ_BODY) 

        const res = await request(api).get(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails.token}`)
    ...
    test("sends 404 status code and error when resource with specified id does not exist", async () => {
        ...
        const res = await request(api).get('/visualizations/1').set("Authorization", `Bearer ${loginDetails.token}`)
    ...
    test("sends 401 status code and error when logged in as incorrect user", async () => {
        ...
        const res = await request(api).get(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails2.token}`)  
...
describe("GET /users/{id}/visualizations - get visualizations of user", () => {
    test("sends array of visualizations and 200 status code", async () => {
        ...
        const createRes2 = await request(api).post('/visualizations').set("Authorization", `Bearer ${loginDetails.token}`).send(VISUAL_POST_REQ_BODY2)
        const expectedArray = [
        ...
        const res = await request(api).get(`/users/${loginDetails.id}/visualizations`).set("Authorization", `Bearer ${loginDetails.token}`)  
    ...
    test("sends 401 status code and error when logged in as incorrect user", async () => {
        ...
        const res = await request(api).get(`/users/${loginDetails.id+1}/visualizations`).set("Authorization", `Bearer ${loginDetails.token}`)    
...
describe("DELETE /visualizations/{id} - delete visualization", () => {
    test("removes from database, sends 200 status code", async () => {
        ...
        const res = await request(api).delete(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails.token}`)
    ...
    test("sends 404 status code and error when resource with specified id does not exist", async () => {
        ...
        const res = await request(api).delete('/visualizations/1').set("Authorization", `Bearer ${loginDetails.token}`)
    ...
    test("sends 401 status code and error when logged in as incorrect user", async () => {
        ...
        const res = await request(api).delete(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails2.token}`)  
...
describe("PATCH /visualizations/{id} - update visualization info", () => {
    test("updates info, sends 200 status code", async () => {
        ...
        const res = await request(api).patch(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails.token}`).send(VISUAL_PATCH_REQ_BODY)  
    ...
    test("sends 404 status code and error when resource with specified id does not exist", async () => {
        ...
        const res = await request(api).patch('/visualizations/1').set("Authorization", `Bearer ${loginDetails.token}`).send(VISUAL_PATCH_REQ_BODY)  
    ...
    test("sends 401 status code and error when logged in as incorrect user", async () => {
        ...
        const res = await request(api).patch(`/visualizations/${createRes.body.id}`).set("Authorization", `Bearer ${loginDetails2.token}`).send(VISUAL_PATCH_REQ_BODY)  
